		...
		<div class="content">
			...
			<div class="right fr">
				<div class="session-form__container">
					<h2 class="session-form__header">登录<span>没有账号？<router-link to="/signup" style="color: #7abdc3;" tag="span">点此注册</router-link></span></h2>
					<div v-if="use">
					...
					<div v-if="mob">
						<el-form :model="ruleForm1" status-icon :rules="rules" ref="ruleForm1" label-width="100px" class="demo-ruleForm">
							...
							<el-form-item>
								<el-button type="primary" @click="submitForm1('ruleForm1')">登 录</el-button>
							</el-form-item>
						</el-form>
					</div>
					<p style="text-align: center;margin-bottom: 20px;cursor: pointer;"><router-link style="color: #005980;" to="/passwordNew">已有账号，忘记密码？</router-link></p>
...
//								url: this.$store.state.api + "api/ALogin/Login",
							...
							.then(
								function(data) {
									...
									if(data.data.ExcuteCode == 0) {
										...
										let {token} = data.data.Data.UserAuthenticationToken
			...
			setCookie(c_name, c_pwd, exdays) {
				...
				//字符串拼接cookie
				window.document.cookie = "ruleForm.name" + "=" + c_name + ";path=/;expires=" + exdate.toGMTString();
				window.document.cookie = "ruleForm.checkPass" + "=" + c_pwd + ";path=/;expires=" + exdate.toGMTString();
			},
			//读取cookie
			getCookie: function() {
				if(document.cookie.length > 0) {
					...
					for(var i = 0; i < arr.length; i++) {
						...
						//判断查找相对应的值
						console.log(arr2)
						if(arr2[0] == 'ruleForm.name') {
							this.ruleForm.name = arr2[1]; //保存到保存数据的地方
						} else if(arr2[0] == 'ruleForm.checkPass') {
			...
			yzm() {
				...
				if(this.ruleForm1.mobile == '' || this.ruleForm1.mobile == null) {
					...
					param.append('SendType', 2)
					this.$axios({
							method: 'post',
//							url: this.$store.state.api + 'api/SmsSend/smsSendAdminLogin',
...
//								url: this.$store.state.api + 'api/ALogin/PhoneLogin',
								url: '/api/api/userInfo/LoginUserSystemByTelephoneCode',
								async: false,
								data: param,
							})
							.then(function(data) {
